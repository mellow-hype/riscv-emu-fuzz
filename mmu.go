...
// An isolated memory space to be used by our called process
type Mmu struct {
	// Block of memory which belongs to this guest
	// Offset 0 corresponds with address 0 in the guest address space
	memory []uint8

	// Holds the permission bytes for the corresponding byte in memory
	permissions []Perm

	// Tracks blocks in memory which are dirty and will need to be reset
	dirty []VirtAddr
...
// Create a new instance of the MMU struct
func newMmu(size uint) *Mmu {
	// Create a new memory space of size `size` bytes
	m := Mmu{
...
// Mmu: Fork an existing MMU instance
func (m *Mmu) fork() *Mmu {
	// Create a new memory space of size `size` bytes
	fmt.Println("\n===== FORKING =======")
	...
	// Copy from current mmu to the clone
	copy(clone.memory, m.memory)
	copy(clone.permissions, m.permissions)
	return &clone
}

// Mmm: method for setting permission bytes for a guest virtual address
...
func (m *Mmu) reset(orig_mmu *Mmu) {
	...
	for _, block := range m.dirty {
		...
		// Set the value of each byte in the dirtied blocks back to the value from orig_mmu
